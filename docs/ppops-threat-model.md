...
- **RAILGUN runtime ↔ RPC/PPOI:** encrypted chain history, receipts, block tags,
  ...
  payment credit closed unless finality and spendability agree; it does not
  verify cryptographic RPC proofs or use quorum for receipt/finality reads.
  Evidence: `src/railgun/engine.ts` and `src/railgun/scanner.ts`.
...
- **Developers/registry → build:** npm packages, GitHub Actions and the Docker
  ...
  CI is read-only, but Actions use mutable major tags and the pinned RAILGUN tree
  retains disclosed transitive vulnerabilities. Evidence: `package-lock.json`,
  `.github/workflows/ci.yml` and `docs/OPERATIONAL-PROFILE.md`.
...
| --- | --- | --- | --- | --- |
| Health endpoint | `GET /v1/health` | local network → daemon | Unauthenticated, minimal readiness only | `src/api/app.ts` / `createApiApp` |
| Operational API | `/v1/intents`, `/settlements`, `/events`, `/outbox` | backend/client → daemon | Single Bearer role; 64 KiB body cap; no rate limiter | `src/api/app.ts`; `src/security/auth.ts` |
...
| RPC finality/receipt reads | Configured JSON-RPC URL | external provider → scanner | Primary RPC only for these reads; no proof/quorum | `src/railgun/scanner.ts` / `chainContext`, `refreshKnownChainState` |
...
| npm/CI/container build | Install or CI run | registry/action/base image → executable | Lockfile/pins and read-only CI permissions; unresolved advisories remain | `package-lock.json`; `.github/workflows/ci.yml`; `Dockerfile` |
...
4. **Manipulate settlement availability/integrity:** operate or compromise the
   selected primary RPC/PPOI endpoint → omit receipts, delay finality or feed
   misleading spendability state → keep legitimate orders unpaid or, if an
   unsafe low-confirmation policy is chosen, amplify reorg risk (TM-004,
   TM-009).
...
| TM-004 | Malicious/failed RPC or PPOI operator; network observer | Operator config selects the service and PPOps has no authenticated proof/quorum | Omit, delay or falsify receipt/finality/PPOI data and observe scan timing | Payment censorship/delay, privacy leakage and potentially unsafe state under weak finality policy | Settlement integrity/availability, request-timing privacy | Exact chain/deployment check; matched + `FINALIZED` + `SPENDABLE` required; unknown bucket fails closed; non-final receipt recheck (`src/railgun/engine.ts`, `src/railgun/scanner.ts`, `src/reconciliation/projection.ts`) | Receipt/finality reads use first RPC; no quorum/proofs; PPOI is inherited trust; URL may identify merchant | Prefer self-hosted or privacy-preserving RPC; compare at least two independent RPCs for receipt/finality; monitor height divergence; document minimum confirmation policy per network | Record metadata-free provider health/height divergence, PPOI bucket age and confirmation latency | Medium: outages/inconsistency are common and malicious observation is inherent | High: can censor fulfillment or undermine payment-state integrity | high |
| TM-005 | npm/action/base-image/artifact supply-chain attacker | A vulnerable/replaced dependency path is reachable during install/build/runtime | Execute code or write outside intended artifact state, then read secrets/alter reconciliation | Complete application confidentiality/integrity compromise, but still no RAILGUN spending key | All process-held secrets/state, build outputs | Exact package lock/pins; Axios override; artifact path containment; non-root read-only container; CI read-only permissions (`package.json`, `src/railgun/engine.ts`, `Dockerfile`, `.github/workflows/ci.yml`) | `npm audit` retains 66 findings including 3 critical; large unused legacy Web3/BZZ/GraphQL tree; Actions/base images not digest-pinned | Obtain upstream dependency pruning/fixes; generate SBOM and reachability review; pin Actions/images by digest; run SDK in separate constrained process/container with egress allowlist; re-run primitive gate on every upgrade | Dependency-diff alerts, lockfile review, image/SBOM scanning, runtime unexpected-module/egress monitoring | Medium: exploit reachability is unproven, but network-facing SDK code and known stale packages increase exposure | High: runtime code execution reaches viewing/signer/API keys and payment state | high |
...
| TM-007 | Network observer with endpoint access, compromised webhook receiver | Valid request is captured or receiver does not enforce replay policy | Replay a signed event or exploit at-least-once retry after receiver success/PPOps failure | Duplicate fulfillment or disclosure of operational amount/status graph to receiver | Webhook key, fulfillment integrity, event metadata | Remote HTTPS required; redirects rejected; HMAC covers timestamp/event ID/raw body; stable event IDs and persisted outbox (`src/events/webhook.ts`, `src/db/database.ts`) | Receiver verification/replay cache is outside repo; no key ID/rotation; HMAC provides no end-to-end confidentiality from receiver | Publish receiver verification sample; enforce ±5 minute timestamp window and durable event-ID dedupe; add key ID/dual-key rotation | Alert on duplicate event IDs, stale timestamps, signature failures, retry/dead-letter spikes | Medium: retries are normal and receiver mistakes are plausible | Medium: affects fulfillment and metadata, not spending authority | medium |
| TM-008 | Remote note spammer or client of an exposed API | Attacker can pay/send notes or reach API; RAILGUN scanning is resource intensive | Generate strict-looking memos, oversized/request bursts or slow connections to consume scan/RPC/DB resources | Delayed confirmation and daemon availability loss | Scanner/API/SQLite availability | 64 KiB bodies; server timeouts/header cap; 8-way normalization cap; one active scan; pagination caps; Docker resource privilege reduction (`src/api/app.ts`, `src/api/server.ts`, `src/railgun/scanner.ts`) | No rate limiter, quotas, DB retention policy or container CPU/memory limit; note decryption cost is inherited | Add proxy rate limits if exposed, Compose CPU/memory/PID limits, unmatched-settlement retention/metrics and scan-lag SLO | Monitor scan duration, TXO/reference counts, unmatched growth, event-loop lag, RSS and API response codes | Medium: note spam is permissionless but costs the attacker; API risk depends on exposure | Medium: availability delay with fail-closed payment state | medium |
| TM-009 | Upstream SDK change, malicious chain data, implementation bug | A TXO/PPOI/finality shape changes or an edge case escapes tests | Cause identity collision, premature credit, regression or duplicate transition | Incorrect order fulfillment or missed/repeated confirmation | Settlement identity, projections, outbox integrity | Pinned versions; immutable identity comparison; SQLite uniqueness/transaction; orthogonal statuses; `bigint`; restart/partial/over/late/reorg/PPOI tests and primitive gate (`src/reconciliation/service.ts`, `src/db/database.ts`, `test/core.test.ts`) | Direct engine TXO API is not a stable high-level contract; no fresh strict live PPOps testnet payment in automated CI; low confirmation values are operator-selectable | Add a maintained strict testnet payment fixture and schema-contract tests against SDK upgrades; require conservative network profile; formalize invariants/property tests | Alert on identity collisions, paid→nonpaid transitions, conflicts and confirmation latency anomalies | Low-to-medium with pinned versions; rises on upgrade or unusual chain behavior | High: false confirmation can cause real-world fulfillment loss | high |
...
- [x] Covered HTTP, CLI, config, secrets, wallet state, chain TXOs, RPC/PPOI,
  artifact paths, webhook, restore and build entry points.
