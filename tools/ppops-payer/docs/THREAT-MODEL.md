...
| Accidental wrong/large payment | Arbitrum/native-USDC pinning, fresh `OPEN` request, exact intent confirmation and independent maximum atomic amount | Operator can deliberately confirm the wrong valid intent |
...
| Transaction response lost after submission | Transaction hash emitted when available; runbook forbids blind retry | An RPC failure after broadcast can leave an ambiguous result requiring nonce/chain inspection |
