...
export class PayerRuntimeLock {
  ...
  static async acquire(path: string): Promise<PayerRuntimeLock> {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        ...
        if (existing && Number.isSafeInteger(existing.pid) && processIsAlive(existing.pid)) {
          throw new Error(`Another ppops-payer process is active with PID ${existing.pid}`);
        }
        await rename(path, `${path}.stale-${Date.now()}`);
      }
    }
    throw new Error("Unable to acquire the ppops-payer runtime lock");
  }
...
export const payerRuntimeLockPath = (walletStatePath: string): string =>
  `${walletStatePath}.runtime-lock`;
